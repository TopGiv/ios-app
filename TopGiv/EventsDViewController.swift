...
class EventsDViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    */
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row != 0 {
            let identifier = "EventCTableViewCell"
            let object = self.userDict[indexPath.row]
            
            var cell: EventCTableViewCell! = tbl_Events.dequeueReusableCell(withIdentifier: identifier) as? EventCTableViewCell
            
            if cell == nil {
                tableView.register(UINib(nibName: "EventCTableViewCell", bundle: nil), forCellReuseIdentifier: identifier)
                cell = tbl_Events.dequeueReusableCell(withIdentifier: identifier) as? EventCTableViewCell
            }
            
            cell.lb_Title.text = object["title"]! as? String
            cell.lb_Date.text = object["date"]! as? String
            cell.im_Events.image = UIImage.init(named: (object["image"]! as? String)!)
            
            return cell
        }
        else {
            let identifier = "EventDTableViewCell"
            let object = self.userDict[indexPath.row]
            
            var cell: EventDTableViewCell! = tbl_Events.dequeueReusableCell(withIdentifier: identifier) as? EventDTableViewCell
            
            if cell == nil {
                tableView.register(UINib(nibName: "EventDTableViewCell", bundle: nil), forCellReuseIdentifier: identifier)
                cell = tbl_Events.dequeueReusableCell(withIdentifier: identifier) as? EventDTableViewCell
            }
